...
def update_dewey_list_options(selected_dewey_level):
    ...
    dashboardGruposUtil: str = "DashboardGruposUtilsDeweyList"
    ...
    dewey_list = requests.get(
        url=url_dewey_list, params={"selected_dewey_level": selected_dewey_level}
    )
...
def update_table(dewey):
    ...
    smartuj_endpoint: str = "localhost:8000/api"
    uso_biblioteca: str = "suj-e-004"
    dashboardGrupos: str = "DashboardGrupos"

    url_grupos: str = (
        "http://" + smartuj_endpoint + "/" + uso_biblioteca + "/" + dashboardGrupos
    )
...
@app.callback(Output("count", "children"), Input("users_table", "data"))
